...
contract DCA is ReentrancyGuard, Pausable, Ownable {
    ...
    function getOrderAt(uint256 index) external view returns (bytes32) {
     ...
     /**
     * @notice Show the order for user.
     * @param user User address.
     * @return The order list.
     */
    function getOrderByUser(address user, uint8 page, uint8 limit) external view returns (Order[] memory) {
        uint256 totalOrders = ordersCreatedByUser[user];
        uint256 lastRecord = page * limit;
        uint256 last = lastRecord > totalOrders ? totalOrders : lastRecord;
        uint256 first = last - limit;
        
        Order[] memory allOrders = new Order[](last - first);

        for (uint256 i = first; i < last; i++) {
            bytes32 orderId = orderIdOfUserByIndex[user][i];
...
/**
    ...
    function findExchangeAndExecute(uint256 amountIn, address[] memory path, address user,uint256 priceIndex, uint16 feeTier) internal returns(uint256 amountOut) {
        ...
        if(priceIndex==0 && path[0] != address(0) && path[1] != address(0)){
            ...
            ISwapRouter.ExactInputSingleParams({
                tokenIn: path[0] == address(0) ? wethaddress : path[0],
                tokenOut: path[1] == address(0) ? wethaddress : path[1],
                fee: feeTier,
                recipient: path[1] == address(0) ? address(this) : user,
                amountIn: amountIn,
                amountOutMinimum: 0,
                sqrtPriceLimitX96: 0
            });
            if (path[0] == address(0)) {
                amountOut = uniswapV3Router.exactInputSingle{ value: amountIn }(params);
            } else {
                amountOut = uniswapV3Router.exactInputSingle(params);
            }
            if (path[1] == address(0) && amountOut > 0) {
    ...
    function _performSwap(bytes32 orderId, uint256 priceIndex, uint16 feeTier) private {
        ...
        if (order.path[0] == address(0)) {
            payable(feeWallet).transfer(dcaFees);
        } else {
        ...
        if (order.path[0] != address(0)) {
            oneToken = 10 ** uint256(IERC20Extended(order.path[0]).decimals());
        }
 ...
 */
    function createOrder(
        ...
        uint256 gasSentByUser = msg.value;
        if (tokenIn == address(0)) {
            gasSentByUser -= amountIn_;
        }
        {
            uint256 gasValue = gasUsedPerTX.mul(tx.gasprice).mul(numOfOrders_);
            _checksForCreateOrder(amountIn_, tokenIn, tokenOut, interval_, numOfOrders_, priceRange_, gasValue, gasSentByUser);
        }

        {
            return _createOrder(amountIn_, tokenIn, tokenOut, interval_, numOfOrders_, priceRange_,msg.sender, gasSentByUser);
...
/**
     ...
     * @notice Find The Profitable routes and perform the swap of tokens
     * @param amountIn Amount of the input token.
     * @param amountOut Amount of the input token.
     * @param path Array of token addresses (path[0] = input token, path[1] = output token).
     * @param priceIndex index to tell us which exchange/router/pair to use.
     * @param feeTier is used when it is a V3 Exchange.
     * @return amountOut of tokens bought
     */
    function swapExactInputSingle(uint256 amountIn, uint256 amountOutMin, address[] memory path, uint256 priceIndex, uint16 feeTier, bool splitRoute, uint256 deadline) external payable returns(uint256 amountOut) {
        require(path.length == 2, "required 2 tokens");
        // Swap ERC20 token for token
        uint256[] memory uniswapV2OutputAmounts;
        address wethaddress = address(0x4200000000000000000000000000000000000006);
        
        if(priceIndex==0 && path[0] != address(0) && path[1] != address(0) && !splitRoute) {
            IERC20(path[0]).safeTransferFrom(msg.sender, address(this), amountIn);
            IERC20(path[0]).safeApprove(address(uniswapV2Router), amountIn);
            uniswapV2OutputAmounts= uniswapV2Router.getAmountsOut(amountIn, path);
            amountOut = uniswapV2OutputAmounts[1];
            uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn, amountOutMin, path, msg.sender, block.timestamp);
        } else {
            if (path[0] != address(0)) {
                IERC20(path[0]).safeTransferFrom(msg.sender, address(this), amountIn);
                IERC20(path[0]).safeApprove(address(uniswapV3Router), amountIn);
            }
            if (splitRoute && path[0] != address(0) && path[1] != address(0)) {
                bytes memory pathBytes = abi.encodePacked(path);
                ...
                ISwapRouter.ExactInputSingleParams memory params =
                    ISwapRouter.ExactInputSingleParams({
                        ...
                        tokenOut: path[1] == address(0) ? wethaddress : path[1],
                        fee: feeTier,
                        recipient: path[1] == address(0) ? address(this) : msg.sender,
                        amountIn: amountIn,
                        amountOutMinimum: amountOutMin,
                        sqrtPriceLimitX96: 0
                    });
                    amountOut = uniswapV3Router.exactInputSingle{ value: msg.value }(params);
                if (path[1] == address(0) && amountOut > 0) {
